...
/**
 * Synchronous SHA-256 (FIPS 180-4) for small inputs where WebCrypto's async
 * API does not fit, such as fingerprinting inside a parser loop.
 */
export function sha256Sync(input: Uint8Array): string {
  ...
  view.setUint32(padded.length - 8, Math.floor((input.length * 8) / 2 ** 32));
